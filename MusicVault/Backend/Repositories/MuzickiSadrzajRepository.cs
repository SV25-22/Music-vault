...
public class MuzickiSadrzajRepository : SQLGenericRepository<MuzickiSadrzaj> {
    public static List<Delo> GetDela(string search = "") {
        ...
        return context.Delo.Include(d => d.MuzickiSadrzaji).Where(d => string.IsNullOrEmpty(search) || d.Opis.ToLower().Contains(search.ToLower())).ToList();
    }

    public static List<Album> GetAlbumi(string search = "") {
        using var context = new SqlDbContext();
        return context.Album.Include(a => a.MuzickiSadrzaji).Where(a => string.IsNullOrEmpty(search) || a.Opis.ToLower().Contains(search.ToLower())).ToList();
    }

    public static List<Nastup> GetNastupi(string search = "") {
        using var context = new SqlDbContext();
        return context.Nastup.Include(n => n.MuzickiSadrzaji).Where(n => string.IsNullOrEmpty(search) || n.Opis.ToLower().Contains(search.ToLower())).ToList();
    }
